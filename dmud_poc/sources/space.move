module dmud_poc::space {
    ...
    struct ZoneTicker has key, store {
        id: UID,
        zone: UID,
        tick: u64,
    }
    ...
    struct LoaderStaticActors has key, store {
        id: UID,
    }
    struct LoaderDynamicActors has key, store {
        id: UID,
    }
    struct LevelTicker has key, store {
        id: UID,
        level: UID,
        tick: u64,
    }
    ...
    // fun new_zone(ctx: &mut TxContext): ID {
    //     let zone = Zone {
    //         id: object::new(ctx),
    //     };
    //     let id = object::uid_to_inner(&zone.id);
    //     transfer::transfer(zone, tx_context::sender(ctx));
    //     id
    // }    

    // fun new_level(ctx: &mut TxContext): UID {
    //     let level = Level {
    //         id: object::new(ctx),
    //     };
    //     move_to_sender(level);
    //     level.id
    // }
